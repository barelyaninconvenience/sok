...
.NOTES
    ...
    Version: 2.0.0 (SOK canonical — formerly TITAN Archiver)
    Date: 18Mar2026
...
foreach ($folder in $SourceFolders) {
    ...
    if (Test-Path $folder) {
        ...
        try {
            $found = Get-ChildItem -Path $folder -Recurse -File -ErrorAction Continue |
...
if ($DryRun) {
    ...
    foreach ($item in $manifest | Select-Object -First 20) {
        Write-SOKLog "  $($item.FullName) ($(Get-HumanSize $item.Length))" -Level Debug
    }
...
try {
    ...
    foreach ($item in $manifest) {
        $name = if ($item.Name.Length -gt 48) { $item.Name.Substring(0, 45) + "..." } else { $item.Name }
        $size = Get-HumanSize $item.Length
    ...
    foreach ($file in $manifest) {
        ...
        $writer.WriteLine("`n# " + ("-" * 118))
        $writer.WriteLine("# FILE: $($file.FullName)")
        $writer.WriteLine("# SIZE: $(Get-HumanSize $file.Length) | MODIFIED: $($file.LastWriteTime.ToString('ddMMMyyyy HH:mm:ss'))")
        $writer.WriteLine("# " + ("-" * 118))
...
$archiveSize = Get-HumanSize (Get-Item $targetFile).Length
