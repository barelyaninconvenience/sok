...
foreach ($cat in $categoryStats.Keys | Sort-Object) {
    ...
    Write-SOKLog "  $($cat.PadRight(25)) Total:$($s.Count.ToString().PadLeft(4))  Mem:$($s.MemMB.ToString().PadLeft(6)) MB$killLabel$protLabel" -Level $level
}
...
elseif ($DryRun) {
    ...
    foreach ($group in $grouped) {
        ...
        foreach ($item in ($group.Group | Sort-Object MemMB -Descending | Select-Object -First 5)) {
            Write-SOKLog "    $($item.Name) (PID $($item.PID), $($item.MemMB) MB) — $($item.Reason)" -Level Debug
        }
        if ($group.Count -gt 5) {
            Write-SOKLog "    ... and $($group.Count - 5) more" -Level Debug
        }
    }
}
else {
