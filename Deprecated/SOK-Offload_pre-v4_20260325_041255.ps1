...
foreach ($target in $offloadTargets) {
    ...
    foreach ($rPath in $resolvedPaths) {
        ...
        if ($sizeKB -ge $MinSizeKB) {
            ...
            Write-SOKLog "  VIABLE: $($target.Label) — $(Get-HumanSize ($sizeKB * 1KB)) ($fileCount files) P$($target.Priority) Safety:$safetyScore" -Level Ignore
        }
        elseif ($sizeKB -gt 0) {
            Write-SOKLog "  BELOW MIN: $($target.Label) — $(Get-HumanSize ($sizeKB * 1KB))" -Level Debug
        }
    }
}
