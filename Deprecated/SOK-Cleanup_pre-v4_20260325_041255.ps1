...
foreach ($proc in $processesToKill) {
    $running = Get-Process -Name $proc.Name -ErrorAction Continue
    if ($running) {
        if ($DryRun) {
...
$toDelete = @(
    ...
    @{ Path = "$env:LOCALAPPDATA\Microsoft\Olk\EBWebView\Default\Cache";              Label = 'Outlook Web Cache' }
...
foreach ($item in $toDelete) {
    ...
    if ($DryRun) {
        Write-SOKLog "[DRY] Would delete: $($item.Label) — $(Get-HumanSize ($sizeKB * 1KB))" -Level Ignore
        $deletedCount++; $deletedKB += $sizeKB
    }
    else {
        Write-SOKLog "Deleting: $($item.Label) — $(Get-HumanSize ($sizeKB * 1KB))" -Level Ignore
        try {
...
foreach ($item in $toOffload) {
    ...
    if ($DryRun) {
        Write-SOKLog "[DRY] Would offload: $($item.Label) — $(Get-HumanSize ($sizeKB * 1KB))" -Level Ignore
        Write-SOKLog "  $($item.Path) → $destPath" -Level Debug
        $movedCount++; $movedKB += $sizeKB
        continue
    }

    Write-SOKLog "Offloading: $($item.Label) — $(Get-HumanSize ($sizeKB * 1KB))" -Level Ignore
    Write-SOKLog "  $($item.Path) → $destPath" -Level Debug
...
Write-SOKLog "  Deleted:    $deletedCount items, $(Get-HumanSize ($deletedKB * 1KB))" -Level Success
Write-SOKLog "  Offloaded:  $movedCount items, $(Get-HumanSize ($movedKB * 1KB))" -Level Success
Write-SOKLog "  Failed:     $failedCount" -Level $(if ($failedCount -gt 0) { 'Error' } else { 'Success' })
Write-SOKLog "  C: now:     $(Get-HumanSize ($newFreeKB * 1KB)) free ($newUsedPct% used)" -Level Ignore
