...
function Get-ProcessCategory {
    ...
    try {
        ...
        # CRITICAL: Core Windows (Session 0 critical processes)
        if ($sessionId -eq 0 -and $name -in @('System','Registry','smss','csrss','wininit','services','lsass','dwm','winlogon','fontdrvhost','LogonUI')) {
            return @{Category='WindowsCore'; Kill=$false; Reason='Session 0 critical'}
        }
        
        # CRITICAL: svchost and services
        if ($name -eq 'svchost' -or ($sessionId -eq 0 -and $name -notlike "*Telemetry*")) {
            return @{Category='WindowsService'; Kill=$false; Reason='Windows service'}
        }
        
        # CRITICAL: Security software
        if (($company -like "*Microsoft*" -and ($name -like "*Defender*" -or $name -like "*MsMp*" -or $name -like "*Niss*")) -or
            $company -like "*Symantec*" -or $company -like "*McAfee*") {
            return @{Category='Security'; Kill=$false; Reason='Security software'}
        }
        
        # CRITICAL: Audio/Video drivers
        if ($company -like "*Realtek*" -or $company -like "*NVIDIA*" -or $company -like "*AMD*" -or 
            $name -eq 'audiodg' -or $name -like "*Audio*") {
            return @{Category='AudioVideo'; Kill=$false; Reason='Audio/Video driver'}
        }
        
        # CRITICAL: Windows shell
        if ($name -in @('explorer','sihost','ShellExperienceHost','StartMenuExperienceHost','dwm','RuntimeBroker')) {
            return @{Category='Shell'; Kill=$false; Reason='Windows shell/runtime'}
        }
        
        # AGGRESSIVE CATEGORIZATION - KILL THESE
        
        # Telemetry (always kill)
        if ($name -like "*Telemetry*" -or $name -like "*Diag*" -or $name -like "*Census*" -or 
            $name -eq 'CompatTelRunner' -or $name -eq 'DiagsCap') {
            return @{Category='Telemetry'; Kill=$true; Reason='Telemetry'}
        }
        
        # Updaters (non-Microsoft)
        if ($name -like "*Update*" -and $company -notlike "*Microsoft*") {
            return @{Category='Updater'; Kill=$true; Reason='Third-party updater'}
        }
        
        # Crash reporters
        if ($name -like "*Crash*" -or $name -like "*Reporter*") {
            return @{Category='CrashReporter'; Kill=$true; Reason='Crash reporter'}
        }
        
        # Cloud sync background
        if ($path -like "*OneDrive*" -or $path -like "*Dropbox*" -or $path -like "*Google*Drive*") {
            if ($Process.MainWindowHandle -eq 0) {
                return @{Category='CloudSync'; Kill=$true; Reason='Cloud sync background'}
            }
        }
        
        # AppData background processes (aggressive)
        if ($path -like "*AppData\Local*" -or $path -like "*AppData\Roaming*") {
            if ($Process.MainWindowHandle -eq 0) {
        ...
        # DEFAULT: AGGRESSIVE - Kill unknown non-system processes
        # If we don't know what it is and it's not in Session 0, kill it
        if ($sessionId -ne 0) {
            return @{Category='UserProcess'; Kill=$true; Reason='User process (aggressive mode)'}
        }
        
        # Fallback: Conservative for Session 0
...
function Update-AllPackageManagers {
    Write-SectionHeader "PACKAGE MANAGER UPDATES"
    
    $updated = @()
    
    # Chocolatey
    if (Get-Command choco) {
        Write-Status "Chocolatey..." "Info"
        if (-not $DryRun) {
            try {
                choco upgrade all -y --limit-output 2>&1
                $updated += "Chocolatey"
                Write-Status "[+] Chocolatey" "Success"
            } catch { Write-Status "[-] Chocolatey failed" "Error" }
        }
    }
    
    # Scoop - FIXED COMMANDS
    if (Get-Command scoop) {
        Write-Status "Scoop..." "Info"
        if (-not $DryRun) {
            try {
                # Update scoop itself
                scoop update 2>&1
                ...
                scoop upgrade * 2>&1
                $updated += "Scoop"
                Write-Status "[+] Scoop" "Success"
            } catch { Write-Status "[-] Scoop failed" "Error" }
        }
    }
    
    # Winget
    if (Get-Command winget) {
        Write-Status "Winget..." "Info"
        if (-not $DryRun) {
            try {
                winget upgrade --all --accept-source-agreements --accept-package-agreements 2>&1
                $updated += "Winget"
                Write-Status "[+] Winget" "Success"
            } catch { Write-Status "[-] Winget failed" "Error" }
        }
    }
    
    # Pip
    if (Get-Command pip) {
        Write-Status "Pip..." "Info"
        if (-not $DryRun) {
            try {
                $outdated = pip list --outdated --format=json 2>$null | ConvertFrom-Json
                foreach ($pkg in $outdated) {
                    pip install --upgrade --quiet $pkg.name 2>&1
                }
                $updated += "Pip($($outdated.Count))"
                Write-Status "[+] Pip: $($outdated.Count) packages" "Success"
            } catch { Write-Status "[-] Pip failed" "Error" }
        }
    }
    
    # npm
    if (Get-Command npm) {
        Write-Status "npm..." "Info"
        if (-not $DryRun) {
            try {
                npm update -g 2>&1
                $updated += "npm"
                Write-Status "[+] npm" "Success"
            } catch { Write-Status "[-] npm failed" "Error" }
        }
    }
    
    # Cargo
    if (Get-Command cargo) {
        if (Get-Command cargo-install-update) {
            Write-Status "Cargo..." "Info"
            if (-not $DryRun) {
                try {
                    cargo install-update -a 2>&1
                    $updated += "Cargo"
                    Write-Status "[+] Cargo" "Success"
                } catch { Write-Status "[-] Cargo failed" "Error" }
            }
        }
    }
    
    Write-Host ""
    Write-Status "Updated: $($updated -join ', ')" "Success"
}
