...
Show-SOKBanner -ScriptName 'SOK-SpaceAudit' -Subheader "Min: $(Get-HumanSize ($MinSizeKB * 1KB)) | Depth: $ScanDepth | Threads: $ThrottleLimit | READ-ONLY"
$logPath = Initialize-SOKLog -ScriptName 'SOK-SpaceAudit'
$startTime = Get-Date

if (-not $OutputDir) {
    $OutputDir = Join-Path $env:USERPROFILE 'Documents\SOK\Audit'
}
...
Write-SOKLog "  Used: $(Get-HumanSize ($cUsedKB * 1KB)) ($cUsedPct%)" -Level $(if ($cUsedPct -gt 88) { 'Error' } elseif ($cUsedPct -gt 66) { 'Warn' } elseif ($cUsedPct -gt 44) { 'Annotate' } else { 'Ignore' })
...
# ═══════════════════════════════════════════════════════════════
# PHASE 2: PARALLEL SIZE CALCULATION (.NET single-pass)
# ═══════════════════════════════════════════════════════════════
Write-SOKLog 'PHASE 2: SIZING (parallel, .NET EnumerateFiles)' -Level Section

$sizeStart  = Get-Date
$sizedDirs  = [System.Collections.Concurrent.ConcurrentBag[object]]::new()
$sizeErrors = [System.Collections.Concurrent.ConcurrentBag[object]]::new()

$dirArray = $allDirs.ToArray()
$totalDirCount = $dirArray.Count

Write-SOKLog "  Sizing $totalDirCount directories across $ThrottleLimit threads..." -Level Ignore

$dirArray | ForEach-Object -Parallel {
    $dirPath  = $_
    $sizedBag = $using:sizedDirs
    ...
    try {
        ...
        $opts.IgnoreInaccessible = $true
        $opts.ReturnSpecialDirectories = $false
        # Match Phase 1: skip junctions during file enumeration to prevent
        # double-counting files reachable through junction redirection
        $opts.AttributesToSkip = [System.IO.FileAttributes]::ReparsePoint -bor [System.IO.FileAttributes]::System
        ...
        $errBag.Add([ordered]@{ Path = $dirPath; Error = $_.Exception.Message; Phase = 'Sizing' })
        return
    }
...
Write-SOKLog "  $($sizedDirs.Count) above threshold in ${sizeDuration}s ($($sizeErrors.Count) errors)" -Level Success
...
foreach ($class in $byStat.Keys) {
    ...
    Write-SOKLog ("  {0}  {1} items  {2}" -f
        $class.PadRight(20),
        $s.Count.ToString().PadLeft(4),
        (Get-HumanSize ($s.SizeKB * 1KB)).PadLeft(12)
    ) -Level $level
}
...
if ($cleanable.Count -gt 0) {
    ...
    foreach ($item in $cleanable) {
        $stale = if ($item.DaysSinceWrite -gt 30) { " (stale: $($item.DaysSinceWrite)d)" } else { '' }
        Write-SOKLog "  $((Get-HumanSize ($item.SizeKB * 1KB)).PadLeft(12))  $($item.Path)$stale" -Level Success
    }
}
...
if ($offloadable.Count -gt 0) {
    ...
    foreach ($item in $offloadable) {
        ...
        Write-SOKLog "  $((Get-HumanSize ($item.SizeKB * 1KB)).PadLeft(12))  $($item.Path)$jNote$dbNote" -Level Ignore
    }
}
...
if ($investigate.Count -gt 0) {
    ...
    foreach ($item in $investigate) {
        $stale = if ($item.DaysSinceWrite -gt 90) { " [STALE: $($item.DaysSinceWrite)d]" } else { '' }
        Write-SOKLog "  $((Get-HumanSize ($item.SizeKB * 1KB)).PadLeft(12))  $($item.Path)$stale" -Level Warn
    }
}
...
if ($keepers.Count -gt 0) {
    ...
    foreach ($item in $keepers) {
        Write-SOKLog "  $((Get-HumanSize ($item.SizeKB * 1KB)).PadLeft(12))  [$($item.Classification.PadRight(18))] $($item.Path)" -Level Debug
    }
}
...
$report | ConvertTo-Json -Depth 5 | Set-Content -Path $reportPath -Force -Encoding UTF8
...
[ordered]@{
    ...
    errors = $allErrors
} | ConvertTo-Json -Depth 5 | Set-Content -Path $errorLogPath -Force -Encoding UTF8
