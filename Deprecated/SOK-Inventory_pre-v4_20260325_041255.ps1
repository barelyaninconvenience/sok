...
Show-SOKBanner -ScriptName 'SOK-Inventory' -Subheader "Depth: $ScanDepth | Multi-Drive | Redundancy: $(if($AllowRedundancy){'ON'}else{'OFF'})"
...
if (-not $OutputPath) {
    $outDir = Join-Path $env:USERPROFILE 'Documents\SOK\Inventory'
    if (-not (Test-Path $outDir)) { New-Item -Path $outDir -ItemType Directory -Force | Out-Null }
    $ts = Get-Date -Format 'yyyyMMdd_HHmmss'
    $OutputPath = Join-Path $outDir "SOK_Inventory_${ts}.json"
}

Write-SOKLog "Scan depth: $ScanDepth | Output: $OutputPath" -Level Ignore
...
try {
    ...
    foreach ($d in $logicalDrives) {
        $level = if ($d.percent_used -gt 88) { 'Error' } elseif ($d.percent_used -gt 66) { 'Warn' } elseif ($d.percent_used -gt 44) { 'Annotate' } else { 'Ignore' }
...
try {
    ...
    $junctionScanRoots += @(Get-ChildItem 'C:\' -Directory -Force -ErrorAction Continue | Select-Object -ExpandProperty FullName)
    ...
    foreach ($root in $junctionScanRoots) {
        ...
        try {
            $dirs = Get-ChildItem -Path $root -Directory -Force -ErrorAction Continue
            foreach ($dir in $dirs) {
...
try {
    $chocoCmd = Get-Command choco -ErrorAction Continue
    if ($chocoCmd) {
    ...
    if ($scoopCmd) {
        ...
        if ($scoopApps.Count -eq 0) {
            ...
            if (Test-Path $scoopAppsDir) {
                $scoopApps = @(Get-ChildItem $scoopAppsDir -Directory -ErrorAction Continue | Where-Object { $_.Name -ne 'scoop' } | ForEach-Object {
        ...
        Write-SOKLog "Scoop $scoopVersion — $($scoopApps.Count) apps, $($scoopBuckets.Count) buckets" -Level Success
    }
    else {
...
# Python
try {
    $pyCmd = Get-Command python -ErrorAction Continue
    if ($pyCmd) {
...
try {
    $nodeCmd = Get-Command node -ErrorAction Continue
    if ($nodeCmd) {
...
# Rust
try {
    $rustCmd = Get-Command rustc -ErrorAction Continue
    if ($rustCmd) {
...
# .NET
try {
    $dotnetCmd = Get-Command dotnet -ErrorAction Continue
    if ($dotnetCmd) {
...
# WinGet
try {
    $wingetCmd = Get-Command winget -ErrorAction Continue
    if ($wingetCmd) {
...
try {
    $dockerCmd = Get-Command docker -ErrorAction Continue
    if ($dockerCmd) {
...
try {
    ...
    foreach ($db in @(
        ...
        $cmd = Get-Command $db.Cmd -ErrorAction Continue
        if ($cmd) {
...
try {
    $gitCmd = Get-Command git -ErrorAction Continue
    if ($gitCmd) {
...
if ($ScanDepth -ge 2) {
    ...
    try {
        ...
        $programs = @($regPaths | ForEach-Object { Get-ItemProperty $_ -ErrorAction Continue } |
...
if (-not $PreviousRunPath) {
    ...
    $prevRuns = Get-ChildItem -Path $prevDir -Filter 'SOK_Inventory_*.json' -ErrorAction Continue | Sort-Object LastWriteTime -Descending
...
$inventory['scan_metadata'] = [ordered]@{
    script_version    = 'SOK-Inventory 3.0.0'
    hostname          = $env:COMPUTERNAME
...
$inventory | ConvertTo-Json -Depth 10 | Set-Content -Path $OutputPath -Force -Encoding UTF8
