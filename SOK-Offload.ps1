...
if (-not (Test-Path "$ExternalDrive\")) {
    if ($DryRun) {
        Write-SOKLog "[DRY] External drive $ExternalDrive not present — preview exits cleanly" -Level Warn
        exit 0
    }
...
foreach ($item in $planItems) {
    ...
    try {
        ...
        #   3 = Files copied + extra files at destination
        #   4 = Mismatched files/dirs detected (warning only)
        #   5 = Files copied + mismatched items
        #   6 = Extra files + mismatched items (no copy needed)
        #   7 = Files copied + extra files + mismatched items
        #   8+ = At least one FAILED copy — actual error condition
        $roboExitMeaning = switch ($roboExit) {
            0 { 'No files copied — already in sync' }
            1 { 'Files copied successfully' }
            2 { 'Extra files/dirs at destination (not an error)' }
            3 { 'Files copied + extra files at destination' }
            4 { 'Mismatched items detected (warning)' }
            5 { 'Files copied + mismatched items' }
            6 { 'Extra files + mismatched items' }
            7 { 'Files copied + extra files + mismatched items' }
            default { "ERROR — copy failure(s) detected (exit $roboExit)" }
        }
        $roboLevel = if ($roboExit -eq 0 -or $roboExit -eq 1) { 'Success' } elseif ($roboExit -lt 8) { 'Warn' } else { 'Error' }
        ...
        if ($roboExit -lt 8) {
            ...
            Write-SOKLog "  Robocopy FAILED — exit $roboExit — $roboExitMeaning" -Level Error
            $failedCount++
        }
    }
    catch {
